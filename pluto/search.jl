...
# ╔═╡ b43755e4-4d87-4999-a6d7-693de2c9b711
md""" $(@bind dosearch Button("Search"))"""
...
function formatted(psgs)
    formatted = []
	for psg in psgs
        urn = string("**", psg.urn.urn, "**")
        txt = replace(psg.text, s => """<span class="hl">$s</span>""")
        push!(formatted, string(urn, " ", txt))
    end
    HTML(join(formatted, "\n"))
end
...
function searchresults(str)
	...
	if isempty(str)
		...
		for urn in generic
			...
			#ιpush!(accentedmatches, urn)
			accented = filter(cn -> dropversion(cn.urn) == urn, c.corpus)
			push!(accentedmatches, accented)
		end
		#accentedmatches |> Iterators.flatten |> collect
		#formatPassages(accentedmatches)
		formatted(accentedmatches)
	end
end

# ╔═╡ 226fc53f-3529-4ef9-b425-5edc70d604e9
md">Don't touch these"
